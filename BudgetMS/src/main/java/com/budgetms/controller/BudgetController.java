...
//budget info
	...
	public Object findBudgetInfoByPage(HttpServletRequest request) {
		...
		BudgetInfo budgetInfo = JSON.toJavaObject(JSON.parseObject(json), BudgetInfo.class);
	...
	public Object deleteBudgetInfo(HttpServletRequest request) {
		String budgetInfoId = request.getParameter("obj");
		JSON json = JSON.parseObject(budgetInfoId);
		// json.
		logger.info("id:" + budgetInfoId);
		try {
			budgetService.deleteBudgetInfo(budgetInfoId);
	...
	public Object findBudgetTypeByPage(HttpServletRequest request) {
		String json = request.getParameter("obj");
		BudgetType budgetType = JSON.toJavaObject(JSON.parseObject(json), BudgetType.class);
		int start = Integer.parseInt(request.getParameter("start"));
		int limit = Integer.parseInt(request.getParameter("limit"));
	...
	public Object deleteBudgetType(HttpServletRequest request) {
		String budgetTypeId = request.getParameter("obj");
		JSON json = JSON.parseObject(budgetTypeId);
		// json.
		logger.info("id:" + budgetTypeId);
		try {
			budgetService.deleteBudgetType(budgetTypeId);
		} catch (DataAccessException e) {
			return MysqlErrTranslator.getJsonErrorMsg(e, logger);
		}
		logger.info(SUCCESS);
